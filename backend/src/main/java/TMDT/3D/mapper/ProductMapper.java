...
public class ProductMapper {
    ...
    public Product mapToProductEntity(ProductDto productDto) {
        ...
        if (productDto.getCategoryId() == null) {
            throw new IllegalArgumentException("Category ID from request must not be null");
        }

        // Lấy Category từ service
        ...
        // Lấy CategoryType từ danh sách categoryTypes của Category
        UUID categoryTypeId = productDto.getCategoryTypeId();
        if (categoryTypeId == null) {
            throw new IllegalArgumentException("CategoryType ID from request must not be null");
        }

        CategoryType categoryType = category.getCategoryTypes().stream()
                .filter(ct -> ct.getId().equals(categoryTypeId))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("CategoryType not found in Category with ID: " + categoryTypeId));
